      ...
      <table class="table table-bordered table-striped css-serial" id="DataTables">
       ...
       <tbody>
         ...
         <tr>
           ...
           <td> {{$dc->no_PRKorWO}}</td>
           <td> {{$dc->unit}}</td> 
           <td> <a href="{{ route('data-monitoring.show', $dc->no_idm)}}" target="_parent">{{$dc->nm_pekerjaan}}</a></td>
           <td> {{$dc->rkap}}</td>
           ...
           <td>
             ...
             <form onsubmit="return confirm('Apa anda yakin ingin menghapus data?');" action="{{route('data-monitoring.destroy', $dc->no_idm)}}" method="post">
               @csrf
               @method('DELETE')
